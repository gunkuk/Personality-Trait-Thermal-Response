...
class Config:
    ...
    out_name: str = "lmm_results_allx_cont_bin.xlsx"
    ...
    add_dir_by_phase_interaction: bool = True
    min_nonnull: int = 30
...
def load_sheet0_and_build_XY(cfg: Config) -> Sheet0Bundle:
    ...
    df0["dir"] = df0["set"].map(set_to_direction)
    df0[time_col] = pd.to_numeric(df0[time_col], errors="coerce")
    df0["phase"] = make_phase(df0[time_col], cfg.static_times, cfg.dynamic_times)
...
# =========================
# Mismatch (Δgap) builder
# =========================
def build_mismatch_delta_gap(df0: pd.DataFrame, bundle: Sheet0Bundle, cfg: Config) -> tuple[pd.DataFrame, List[str]]:
    """
    mismatch(Y): Δgap1~Δgap8 (연번×set×dir 1행)
    Δgap_i = (Mi - Pi)_dynamic_mean - (Mi - Pi)_static_mean
    """
    ...
    d = df0.dropna(subset=[id_col, "set", "dir", "phase"]).copy()
    d[p_cols + m_cols] = to_numeric_df(d, p_cols + m_cols, cfg.missing_tokens)

    pm = d.groupby([id_col, "set", "dir", "phase"], as_index=False)[p_cols + m_cols].mean()

    wide = pm.pivot(index=[id_col, "set", "dir"], columns="phase", values=p_cols + m_cols)
...
# Modeling
# =========================
def build_formula(y: str, x_personality: List[str], x_common: List[str], cfg: Config) -> str:
    """
    MixedLM:
      y ~ personality + demo + (optional survey) + dir + time + C(set) + C(phase) + dir:C(phase)
    """
    terms: List[str] = []
    terms += x_personality
    for c in x_common:
    ...
    if "set" in x_common:
        rhs += " + C(set)"
    if "phase" in x_common:
        rhs += " + C(phase)"
    if cfg.add_dir_by_phase_interaction and ("dir" in x_common) and ("phase" in x_common):
        rhs += " + dir:C(phase)"
...
def fit_mixedlm_one(df: pd.DataFrame, y: str, formula: str, group_col: str, cfg: Config) -> pd.DataFrame:
    ...
    try:
        model = smf.mixedlm(formula, dd, groups=dd[group_col], re_formula="1")
...
def main(cfg: Config) -> None:
    ...
    df0["phase"] = df0["phase"].astype("category")
    ...
    for c in ["dir", bundle.time_col, "set", "phase"]:
        if c in df0.columns:
            x_common.append(c)
    ...
    for y in bundle.y_raw:
        f = build_formula(y, x_person_cont, x_common, cfg)
    ...
    rows = []
    for y in bundle.y_raw:
        f = build_formula(y, x_person_bin, x_common, cfg)
        rows.append(fit_mixedlm_one(df0, y, f, bundle.id_col, cfg))
    ...
    if not mismatch_tbl.empty and gap_cols:
        ...
        rows = []
        for y in gap_cols:
            f = build_formula(y, x_person_cont, x_common_mm, cfg)
        ...
        rows = []
        for y in gap_cols:
            f = build_formula(y, x_person_bin, x_common_mm, cfg)
            rows.append(fit_mixedlm_one(mm, y, f, bundle.id_col, cfg))
