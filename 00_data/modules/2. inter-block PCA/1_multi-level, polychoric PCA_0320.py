# 요약: R에서 생성한 block별 correlation matrix를 읽어 PCA를 수행하고,
# KMO/Bartlett/설명분산/대표변수 proxy를 계산한 뒤 Excel 요약본, PNG, TXT를 저장한다.
...
R_CORR_DIR = PROJECT_ROOT / "model" / "OUT" / "corr_matrices_from_R"
MANIFEST_CSV = R_CORR_DIR / "manifest.csv"

OUTPUT_DIR = PROJECT_ROOT / "model" / "OUT" / f"{SCRIPT_DIR.name}_from_R_corr"
...
# 4) 진단 함수
# =========================
def bartlett_from_corr(corr: pd.DataFrame, n: int) -> Tuple[float, float, int]:
    p = corr.shape[0]
    if n <= p or p < 2:
...
def kmo_from_corr(corr: pd.DataFrame) -> Tuple[float, pd.Series]:
    if corr.shape[0] < 2:
    ...
    try:
        ...
        return np.nan, pd.Series(np.nan, index=corr.columns)
    ...
    return float(kmo_overall), pd.Series(kmo_vars, index=corr.columns)


def validity_judgement_from_corr(x: pd.DataFrame, corr: pd.DataFrame, block_name: str, native_unit: str, corr_method: str) -> Dict[str, object]:
    n, p = x.shape
    ...
    bart_chi2, bart_p, bart_dof = bartlett_from_corr(corr, n)
    kmo, kmo_vars = kmo_from_corr(corr)
    ...
    if corr_method.startswith("polychoric") or corr_method.startswith("mixedCor"):
        cautions.append("R psych 패키지 기반 상관행렬(polychoric / mixedCor)을 사용함.")
    if binary_like > 0:
        cautions.append(f"이 블록에는 이진형/준이진형 변수가 {binary_like}개 포함됨.")
    if ordinal_like == p:
        cautions.append("모든 문항이 Likert/ordinal일 가능성이 높음.")
    if p < 3:
        cautions.append("변수 수가 2개 이하인 블록은 PCA보다는 상관/단일축 비교에 가까움.")
    if n < max(50, 5 * p):
        cautions.append(f"유효 표본수 n={n}가 충분히 크지 않을 수 있음.")
...
class PCAResult:
    ...
    information_loss_1var: float | None
...
def pca_from_corr(corr: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    evr = vals / vals.sum()
    return vals, vecs, evr


def run_block_pca(block_name: str, cols: List[str], sheets: Dict[str, pd.DataFrame], manifest: pd.DataFrame) -> PCAResult:
    ...
    row = manifest.loc[manifest["block"] == block_name]
    if row.empty:
        raise FileNotFoundError(f"Manifest entry not found for block: {block_name}")

    corr_method = row.iloc[0]["corr_method"]
    corr_csv = Path(row.iloc[0]["corr_csv"])

    if not corr_csv.exists():
        raise FileNotFoundError(f"Correlation CSV not found: {corr_csv}")

    corr = pd.read_csv(corr_csv, index_col=0)
    corr = corr.loc[corr.index, corr.columns].astype(float)

    # x와 corr의 변수 순서 맞추기
    common_cols = [c for c in x.columns if c in corr.index]
    x = x[common_cols].copy()
    corr = corr.loc[common_cols, common_cols].copy()

    validity = validity_judgement_from_corr(x, corr, block_name, native_unit, corr_method)
    ...
    eigvals, eigvecs, evr = pca_from_corr(corr)
    cum = np.cumsum(evr)
    ...
    # approximate scores 계산용 observed z-score
    ...
    for i, c in enumerate(x.columns):
        lr = LinearRegression().fit(X_std[:, [i]], target)
        pred = lr.predict(X_std[:, [i]])
        ...
        if r2 > best_r2:
            best_var = c
            best_r2 = float(r2)
            contrib = ", ".join([f"PC{k+1} loading={loadings.loc[c, f'PC{k+1}']:.3f}" for k in range(retained)])
            best_note = f"{c}가 retained PCs({retained}개)를 단일 변수로 설명하는 pseudo-R²={r2:.3f}; {contrib}"

    proxy_ok = best_r2 >= 0.70
    info_loss = 1 - best_r2 if pd.notna(best_r2) else np.nan

    summary_lines = [
        f"[BLOCK] {block_name}",
        f"- native unit: {native_unit}",
        f"- corr method: {corr_method}",
        f"- n={n}, p={p}",
        ...
        f"- best raw proxy: {best_var} (R²={best_r2:.3f}, information_loss={info_loss:.3f})" if best_var else "- best raw proxy: NA",
        f"- detail: {best_note}",
    ]
    ...
    return PCAResult(
        ...
        information_loss_1var=info_loss,
...
def save_block_png(result: PCAResult) -> None:
    ...
    note = (
        ...
        f"KMO={result.validity['kmo']:.3f}, method={result.corr_method}"
        if not pd.isna(result.validity["kmo"])
        else f"n={result.n}, p={result.p}, valid={valid_text}, KMO=NA, method={result.corr_method}"
    )
...
def write_outputs(results: List[PCAResult]) -> None:
    ...
    excel_path = OUTPUT_DIR / "PCA_block_summary_from_R_corr.xlsx"
    with pd.ExcelWriter(excel_path, engine="openpyxl") as writer:
        for res in results:
            summary_rows.append({
                ...
                "information_loss_1var": res.information_loss_1var,
        ...
        summary_df = pd.DataFrame(summary_rows).sort_values(
            by=["valid_for_exploratory_pca", "block"], ascending=[False, True]
        )
        summary_df.to_excel(writer, sheet_name="SUMMARY", index=False)

    txt_path = OUTPUT_DIR / "PCA_block_interpretation_from_R_corr.txt"
...
def main() -> None:
    ...
    print(f"[INFO] R_CORR_DIR: {R_CORR_DIR}")
    ...
    for block_name, cols in BLOCKS.items():
        ...
        res = run_block_pca(block_name, cols, sheets, manifest)
        results.append(res)
        save_block_png(res)

    write_outputs(results)
    print("[DONE] Files saved:")
    print(f"- {OUTPUT_DIR / 'PCA_block_summary_from_R_corr.xlsx'}")
    print(f"- {OUTPUT_DIR / 'PCA_block_interpretation_from_R_corr.txt'}")
    print(f"- {PNG_DIR}")
